...
def test_elexon_neso_normal_day() -> None:
    ...
    assert neso_dim(date(2025, 12, 1), 1)["start_time"] == datetime(
        2025, 12, 1, 0, 0, tzinfo=UTC
    )
    assert neso_dim(date(2025, 12, 1), 48)["start_time"] == datetime(
        2025, 12, 1, 23, 30, tzinfo=UTC
    )
...
def test_elexon_neso_bst_day() -> None:
    ...
    assert neso_dim(date(2025, 7, 1), 3)["start_time"] == datetime(
        2025, 7, 1, 0, 0, tzinfo=UTC
    )
...
def test_elexon_neso_spring_dst() -> None:
    ...
    assert neso_dim(date(2025, 3, 30), 1)["start_time"] == datetime(
        2025, 3, 30, 0, 0, tzinfo=UTC
    )
    assert neso_dim(date(2025, 3, 30), 3)["start_time"] == datetime(
        2025, 3, 30, 1, 0, tzinfo=UTC
    )  # 02:00 BST
    assert neso_dim(date(2025, 3, 30), 5)["start_time"] == datetime(
        2025, 3, 30, 2, 0, tzinfo=UTC
    )
    assert neso_dim(date(2025, 3, 30), 46)["start_time"] == datetime(
        2025, 3, 30, 22, 30, tzinfo=UTC
    )
...
def test_elexon_neso_autumn_dst() -> None:
    ...
    assert neso_dim(date(2025, 10, 26), 1)["start_time"] == datetime(
        2025, 10, 25, 23, 0, tzinfo=UTC
    )  # prev UTC day
    assert neso_dim(date(2025, 10, 26), 3)["start_time"] == datetime(
        2025, 10, 26, 0, 0, tzinfo=UTC
    )  # 01:00 BST
    assert neso_dim(date(2025, 10, 26), 5)["start_time"] == datetime(
        2025, 10, 26, 1, 0, tzinfo=UTC
    )  # 01:00 GMT
    assert neso_dim(date(2025, 10, 26), 50)["start_time"] == datetime(
        2025, 10, 26, 23, 30, tzinfo=UTC
    )
