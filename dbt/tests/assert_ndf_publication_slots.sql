-- Observed NDF cadence, not a guarantee of the number of targets per publication.
-- Leave two UTC days for ingestion and the scheduled mart build to catch up.
{{ config(severity='warn') }}

with expected as (
    select generate_series(
        timestamptz '2026-07-20 00:00+00',
        (date_trunc('day', current_timestamp at time zone 'UTC') at time zone 'UTC')
            - interval '1 day 30 minutes',
        interval '30 minutes'
    ) as publication_slot
),
observed as (
    select distinct
        date_bin(interval '30 minutes', publish_time, timestamptz '2024-01-01 00:00+00') as publication_slot
    from {{ ref('fct_demand_forecast_publication') }}
    where publish_time >= timestamptz '2026-07-20 00:00+00'
)
select e.publication_slot
from expected e
left join observed o using (publication_slot)
where o.publication_slot is null
