...
Two cadences: the marts every six hours without the regional one, and the whole graph with all 202 tests at 01:00. The frequent run selects the marts rather than excluding the expensive model, because the cost is not in the mart at all. A full build spends 1,967 seconds of database time and 896 of those are tests against the fifteen million row regional generation view, so excluding the mart alone saves 11%. Staging is materialised as views and never needs rebuilding, and selecting only the marts brings the run to 202 seconds.

**How it went:** the image built with dbt on the first attempt. A full materialisation through the container came back with 207 of its 217 nodes green and 10 warnings, all of them source gaps already documented in probe findings, and the six-hourly job then ran clean. The marts now carry snapshots ingested at 18:03 rather than whatever a laptop last produced.
...
**How it actually ended.** The restart did not recover it. Within minutes the daemon relaunched five catch-up runs, swap filled completely, available memory fell to 41 MiB, and `systemctl stop docker` hung. The machine was resized from 1 GiB to 4 GiB, rebuilt, and ingestion resumed on the first tick afterwards at 12:03, ten hours after it stopped.

**Recovery was almost entirely automatic.** On the first run back, the 24 hour catch-up window closed the gap on imbalance, market index and demand outturn without intervention. That window was written after the August outage, when the same three sources had to be re-fetched by hand because the code existed but had not been deployed. This time it did its job. Carbon intensity national and regional needed a manual `run_backfill` over the day, regional because it is excluded from the sweep by design and would never have healed on its own. All sources were back to zero missing periods by 12:13.
